...
const RideItem = ({ fullName, fromLocation, toLocation, date, time, cost, vacantPlaces, comments, profilePicUrl, phoneNumber, groupImageUrl }) => {
    ...
    const handlePhonePress = () => {
        const options = ['Chat on WhatsApp', 'Call Phone Number', 'Send SMS', 'Cancel'];
    ...
    const formatPhoneNumber = (phoneNumber) => {
        ...
        // If the number starts with a local format (e.g., 054), prepend the country code for Israel
    ...
    const handleWhatsApp = () => {
        const url = `https://wa.me/${formatPhoneNumber(phoneNumber)}`;
    ...
    const handleCall = () => {
        const url = `tel:${formatPhoneNumber(phoneNumber)}`;
    ...
    const handleSMS = () => {
        const url = `sms:${formatPhoneNumber(phoneNumber)}`;

        Linking.canOpenURL(url)
            .then((supported) => {
                if (!supported) {
                    console.error('Can\'t handle URL:', url);
                    alert('Unable to open the SMS app. Please check the phone number and try again.');
                } else {
                    return Linking.openURL(url);
                }
            })
            .catch((err) => {
                console.error('An error occurred:', err);
                alert('An error occurred while trying to open the SMS app.');
            });
    };
...
//     useEffect(() => {
//         if (expanded) {
//             // Preload the profile image when expanded
//             Image.prefetch(profileImage.uri);
//         }
//     }, [expanded]);
